...
uint32 cr3tab[] = { /* CRC polynomial 0xedb88320 */
...
#define updcrc32(b, c) (cr3tab[((int)(c) ^ ((b)&0xff)) & 0xff] ^ (((c) >> 8) & 0x00FFFFFF))

uint32 data_crc(const void*ptr_,int size)
{
  const unsigned char*ptr = ptr_;
  uint32 crc32 = CRC32INIT;

  while (size--)
    {
/*
  crc32 = updcrc32(*(ptr++),crc32);
*/

      crc32 = (*(uint32*)   (((unsigned char*)cr3tab) + (((crc32&0xff) ^ *(ptr++))<<2))) ^ (crc32 >> 8);
    }
