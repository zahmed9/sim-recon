...
*/
{
    switch (blkId) {
       ...
       default:
            fprintf(stderr,
             "mcfio_GetBlockName: Unrecognized block I.D. %d\n", blkId);
                      
  }         
	
}

void mcfioC_DefineUserBlock(int blkId, char *name, bool_t xdr_filter,
                               int *current_size)
/*
**Let the user define it's own XDR filter.
*/
{
   printf ("Please us the NTuple mechanism. See mcfio_DeclareNtuple. \n");
} 
