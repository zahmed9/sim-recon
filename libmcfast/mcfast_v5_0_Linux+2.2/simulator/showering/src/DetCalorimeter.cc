			  ...
			  const int    numberOfSegments[2] )
{
   ...
   m_gridOfCells = new CalorSignalMC** [m_numberOfSegments[0]] ;
   int i ;   
   for ( i=0; i<m_numberOfSegments[0]; i++ )
   {
      m_gridOfCells[i] = new CalorSignalMC* [m_numberOfSegments[1]] ;
      for ( int j=0; j<m_numberOfSegments[1]; j++ )
      {
         m_gridOfCells[i][j] = new CalorSignalMC() ;
      }
   }
...
Calorimeter::resetResolutionFactor( MCParticle& particle, DFLOAT xSmear )
{
   ...
   {
    report( ::ERROR )
    << "Calorimeter::resetResolutionFactor : trying to reset resolution illegally"
    << "\n" ; 
    return ;
   }
...
//
   ...
   if ( m_gridOfCells == 0 )
   {
      ...
      ((Calorimeter*)this)->m_gridOfCells = new CalorSignalMC** [m_numberOfSegments[0]] ;
      int i, j ;
      for ( i=0; i<m_numberOfSegments[0]; i++ )
      {
         m_gridOfCells[i] = new CalorSignalMC* [m_numberOfSegments[1]] ;
   ...
   if ( m_gridOfCells[ cellAddress[0] ][ cellAddress[1] ]->getE() <= 0 )
   ...
   m_gridOfCells[cellAddress[0]][cellAddress[1]]->update( signal, particle.hepIndexF(), trackStatus ) ;
...
Calorimeter::findCell1( MCParticle& particle ) const
{
   ...
   else if ( m_segmentationType == 3 )
   {
      // r-phi segmentation; find ir 
      DFLOAT radius = sqrt( particle.wtrack().x*particle.wtrack().x
                          + particle.wtrack().y*particle.wtrack().y ) ;
      address = int( ( radius - m_limits[0] ) / m_cellSize[0] ) ;
   }
   if ( address == m_numberOfSegments[0] ) address -= 1 ;
...
Calorimeter::findCell2( MCParticle& particle ) const
{
   ...
   else if ( m_segmentationType == 1 )
   {
      ...
      if ( address == m_numberOfSegments[1] ) address -= 1 ;
   }
...
Calorimeter::segmentationType() const
{
   ...
   if ( m_segmentationType == 3 ) return "R-Phi" ;
   return "X-Y" ;
}
...
// Initial revision
//
// Revision 1.27  1999/10/06 21:21:53  yarba_j
...
//
// Revision 1.26  1999/09/24  20:18:32  yarba_j
// more cleanup (agreed with Andreas on naming)
//
// Revision 1.25  1999/09/23  22:31:10  yarba_j
// now using CalorSignalMC for hit generation + minor modifications
//
// Revision 1.23  1999/05/06  22:33:57  yarba_j
// minor cleanup + some speed improvement (no need to generate showers in an absorbers if it has no neighbours
//
// Revision 1.22  1999/03/24  18:50:35  yarba_j
// trate box-shaped volumes with inner gaps properly, using fisrt version of AlignedBoxWithGap
//
// Revision 1.21  1999/02/22  23:15:47  yarba_j
