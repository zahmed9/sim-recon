...
void prgeom(FILE* fl)
{
    ...
    fprintf(fl,"AbsorberBox:     %3d declared.\n",*n_obj_absorberbox);
    for (i=0;i<*n_obj_absorberbox;i++) {
      fprintf(fl,"  %-6s %s Mat=%s xlimit=%5.3G,%-5.3G ylimit=%5.3G,%-5.3G z0=%5.3G zlen=%5.3G\n",
      absorberbox[i].name,absorberbox[i].shape,absorberbox[i].material,
      absorberbox[i].xlimit[0],absorberbox[i].xlimit[1],
      absorberbox[i].ylimit[0],absorberbox[i].ylimit[1],
      absorberbox[i].z0,absorberbox[i].zlen);
    }
